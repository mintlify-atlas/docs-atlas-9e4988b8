...
response = requests.get('/api/districts/', params={'province_name': 'Bagmati'})

# Less efficient - fetching all districts then filtering
response = requests.get('/api/districts/')
...
try:
    response = requests.get('/api/provinces/', timeout=5)
...
function getDistrictsByProvince(provinceName) {
  ...
  // Make request
  return fetch(`/api/districts/?province_name=${encodeURIComponent(provinceName)}`)
    .then(response => {
